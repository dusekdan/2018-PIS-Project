    ...
    <div class="row">
        ...
        <div class="col-lg-6" style="border-right: 1px solid lightgray">
            ...
            <script>
                ...
                $(function () {
                    $startPicker = $("#menu-validity-start");
                    $startPicker.datepicker({
                        ...
                        defaultDate: new Date("{{$menu->validityStart}}"),
                        dateFormat: dateFormatForDatepickers
                    });
                    $startPicker.datepicker('setDate', new Date("{{$menu->validityStart}}"))
                });
                $(function () {
                    var $endPicker = $("#menu-validity-end")
                    ...
                    $endPicker.datepicker('setDate', new Date("{{$menu->validityEnd}}"))
                });
            </script>

            <form method="post" action="{{route('admin.menu-editor.edit', ['id' => $menu->id])}}">
                ...
                <div class="form-group">
                    ...
                    <input
                            ...
                            class="form-control"
                            required="required"
                            value="{{empty(old('menu-name')) ? $menu->name : old('menu-name')}}"
                    >
                </div>
                    ...
                    >
                        ...
                        @foreach($orderables as $orderable)
                            @if ($orderable->orderable_type->name == 'Polévka')
                                <option value="{{$orderable->id}}" {{  empty(old('menu-soup')) ? ($orderable->id == $menu->soup->id ? "selected=\"selected\"" : "") : (old('menu-soup') == $orderable->id) ? "selected=\"selected\"": ""}}>{{$orderable->name}}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
                    ...
                    >
                        ...
                        @foreach($orderables as $orderable)
                            @if ($orderable->orderable_type->name != 'Polévka' && $orderable->orderable_type->name != 'Pití')
                                <option value="{{$orderable->id}}" {{  empty(old('menu-meal-1')) ? ($orderable->id == $menu->meal_1->id ? "selected=\"selected\"" : "") : (old('menu-meal-1') == $orderable->id) ? "selected=\"selected\"": ""}}>{{$orderable->name}}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    >
                        ...
                        @foreach($orderables as $orderable)
                            @if ($orderable->orderable_type->name != 'Polévka' && $orderable->orderable_type->name != 'Pití')
                                <option value="{{$orderable->id}}" {{  empty(old('menu-meal-2')) ? ($menu->meal_2 != null && $orderable->id == $menu->meal_2->id ? "selected=\"selected\"" : "") : (old('menu-meal-2') == $orderable->id) ? "selected=\"selected\"": ""}}>{{$orderable->name}}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    >
                        ...
                        @foreach($orderables as $orderable)
                            @if ($orderable->orderable_type->name != 'Polévka' && $orderable->orderable_type->name != 'Pití')
                                <option value="{{$orderable->id}}" {{  empty(old('menu-meal-3')) ? ($menu->meal_3 != null && $orderable->id == $menu->meal_3->id ? "selected=\"selected\"" : "") : (old('menu-meal-3') == $orderable->id) ? "selected=\"selected\"": ""}}>{{$orderable->name}}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    <input type="submit" value="Upravit menu" class="btn btn-primary">
                </div>
            </form>
        </div>
