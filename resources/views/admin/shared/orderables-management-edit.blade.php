...
@section('subpage-contents')
    ...
    <div class="row">
        ...
        <div class="col-lg-6" style="border-right: 1px solid lightgray">
            ...
            <h4>Upravit objednatelnou položku</h4>

            <form method="post" action="{{route('admin.orderables-management.edit', ['id' => $orderable->id])}}">
                ...
                <div class="form-group">
                    ...
                    <input
                            ...
                            class="form-control"
                            required="required"
                            value="{{empty(old('orderable-name')) ? $orderable->name : old('orderable-name')}}"
                    >
                </div>
                ...
                <div class="form-group">
                    ...
                    <input
                            ...
                            class="form-control"
                            required="required"
                            value="{{empty(old('orderable-quantity')) ? $orderable->quantity : old('orderable-quantity')}}"
                    >
                ...
                <div class="form-group">
                    ...
                    <input
                            ...
                            class="form-control"
                            required="required"
                            value="{{empty(old('price')) ? $orderable->price : old('price')}}"
                    >
                ...
                <div class="form-group">
                    ...
                    <select name="orderable-type" id="orderable-type" title="orderable-type" class="form-control" required="required">
                        @foreach ($types as $type)
                            <option
                                    value="{{$type->id}}"
                                    {{ empty(old('orderable-type')) ? ($type->id == $orderable->orderable_type_id ? 'selected' : '') :(old('orderable-type') == $type->id ? 'selected' : '') }}
                            >
                                {{$type->name}}
                            </option>
                        @endforeach
                ...
                {{ csrf_field() }}
                <div class="form-group">
                    <button type="submit" class="btn btn-primary">Upravit položku</button>
                </div>
            </form>
